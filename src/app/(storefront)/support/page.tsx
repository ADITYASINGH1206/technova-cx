...
export default function SupportChatPage() {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-80px)] max-w-4xl mx-auto p-4 md:p-6">
      ...
      <div className="flex items-center justify-between p-4 mb-4 rounded-xl bg-white/80 border border-slate-200 backdrop-blur-md shadow-sm">
        <div className="flex items-center space-x-3">
          <div className="w-3 h-3 rounded-full bg-emerald-500 animate-pulse" />
          <div>
            <h1 className="font-semibold text-slate-900">NexaBot AI Support</h1>
            <p className="text-xs text-slate-500">Powered by Orchestrator & Critic Pipeline</p>
          </div>
        </div>
            ...
            <div
              className={`max-w-[85%] md:max-w-[75%] p-4 rounded-2xl ${
                msg.sender === 'user'
                  ...
                  : 'bg-white/90 border border-slate-200 text-slate-800 rounded-bl-none shadow-sm'
              }`}
            >
              ...
              {msg.citations && msg.citations.length > 0 && (
                <div className="mt-3 pt-3 border-t border-slate-200/80 flex flex-wrap gap-1.5">
                  <span className="text-[11px] text-slate-500 self-center mr-1">Grounded in:</span>
                  {msg.citations.map((cite, idx) => (
                    <a
                      ...
                      className="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-medium bg-slate-100 text-blue-600 dark:text-blue-400 hover:bg-slate-200 transition-colors border border-slate-200"
                    >
                      🛡️ {cite.title}
                    </a>
                  ))}
              ...
              {msg.sender === 'bot' && msg.intent !== 'SUPPORT_GREETING' && (
                <div className="mt-3 pt-2 border-t border-slate-200/50">
                  <button 
                    ...
                    className="flex items-center space-x-1 text-[11px] font-mono text-slate-500 hover:text-emerald-500 dark:hover:text-emerald-400 transition-colors"
                  >
                  ...
                  {expandedTraceId === msg.id && (
                    <div className="mt-3 p-3 bg-slate-50 rounded-lg border border-slate-200 font-mono text-[10px] sm:text-xs overflow-hidden">
                      <div className="mb-2">
                      ...
                      <div className="mb-2">
                        ...
                        <div className="pl-3 mt-1 text-slate-700 space-y-1">
                          {msg.traceLog ? (
                      ...
                      <div>
                        ...
                        <div className="pl-3 mt-1 text-slate-700">
                          {msg.criticVerdict ? (
                            <>
                              <div>Verification Verdict: <span className={msg.criticVerdict.verdict === 'pass' ? 'text-emerald-600 dark:text-emerald-400 font-bold' : 'text-red-600 dark:text-red-400 font-bold'}>{msg.criticVerdict?.verdict?.toUpperCase() || 'UNKNOWN'}</span></div>
      ...
      <form onSubmit={(e) => handleSend(e)} className="flex space-x-2">
        ...
        <button
          ...
          className="bg-blue-600 hover:bg-blue-500 text-white font-medium px-6 py-3 rounded-xl text-sm transition-all shadow-lg shadow-blue-600/20 disabled:opacity-50 flex items-center justify-center"
        >
          Send
        </button>
      </form>
